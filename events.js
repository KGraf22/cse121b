...
function removeTask(taskElement) {
    //note the use of Array. filter to remove the elementfrom our task array
    tasks = tasks.filter(
...
function completeTask(taskElement) {
    ...
    const taskIndex = tasks.findIndex(
        (task) =>task.detail === taskElement.childNodes[0]
    );
    // once we hae the index we can modify the complete field.
...
function manageTasks(event) {
    ...
    // event.target will point to the actual icon clicked on. We need to get the parent li to work with however. HINT: Remember element.clostst()? Look it up if you don't
    //because we addad 'data-function="delete"' to each icon in a task we can access a dataset property on our target
